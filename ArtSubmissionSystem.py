...
class ArtSubmissionSystem(Extension):
    ...
    async def art_suggestion_update_data(self, ctx: ComponentContext):
        ...
        initialPost = self.bot.get_channel(config["ArtValidationChannelId"]).get_post(ctx.message.channel.id).initial_post
